...
func TestRunOutputsBookmarksSinceDate(t *testing.T) {
	fetcher := &stubFetcher{
		bookmarks: []hateb.Bookmark{
	...
	code := Run([]string{"--user", "sample", "--since", "2026-05-17"}, &stdout, &stderr, fetcher)
...
func TestRunOutputsBookmarksWithoutSince(t *testing.T) {
	fetcher := &stubFetcher{
		bookmarks: []hateb.Bookmark{
...
func TestRunRejectsInvalidSince(t *testing.T) {
	var stdout, stderr bytes.Buffer

	code := Run([]string{"--user", "sample", "--since", "2026/05/17"}, &stdout, &stderr, &stubFetcher{})

	if code != 2 {
		t.Fatalf("Run returned %d, want 2", code)
	}
	if !strings.Contains(stderr.String(), "--since must be YYYY-MM-DD") {
		t.Fatalf("stderr = %q, want since error", stderr.String())
	}
}
...
func TestRunReportsFetchError(t *testing.T) {
	...
	code := Run(
		[]string{"--user", "sample", "--since", "2026-05-17"},
		&stdout,
		&stderr,
